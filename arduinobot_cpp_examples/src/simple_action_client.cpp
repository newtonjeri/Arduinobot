...
namespace arduinobot_action_client
{
    class SimpleActionClient : public rclcpp::Node{
        public:
            explicit SimpleActionClient(const rclcpp::NodeOptions& options): Node("simple_action_client", options) {
                action_client = rclcpp_action::create_client<arduinobot_msgs::action::Fibonacci>(this, "fibonacci");
                timer = create_wall_timer(1s, std::bind(&SimpleActionClient::timerCallback, this));
              }

        private:
            rclcpp_action::Client<arduinobot_msgs::action::Fibonacci>::SharedPtr action_client;
...
RCLCPP_COMPONENTS_REGISTER_NODE(arduinobot_action_client::SimpleActionClient)